...
// ── the headline face ───────────────────────────────────────────────────────
// A 5×7 face for the CONSOLE'S OWN text — shelf titles, tile captions, the
// info card. Carts never see it: `print` stays 3×5, because a cart's layout
// arithmetic is built on 4-pixel characters and must never move under it.
// The 3×5 face is honest at HUD size and mush at headline size; this is the
// other half of the pair.
//
// Five bits per row, most significant bit leftmost, seven rows. Advance is 6
// (five of glyph, one of air); a line wants 9.

pub const HEADLINE_WIDTH: i32 = 6;

pub fn headline_glyph(c: char) -> Option<[u8; 7]> {
...
pub fn headline_table() -> Vec<(char, [u8; 7])> {
    " ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-.,!?':/"
        .chars()
        .filter_map(|c| headline_glyph(c).map(|rows| (c, rows)))
        .collect()
}
